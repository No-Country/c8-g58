...
import dni from '../../assets/imagenes/dni.svg';
import telefono from '../../assets/imagenes/telefono.svg';
import calendario from '../../assets/imagenes/calendario.svg';

function validate(user) {
	let error = {};

	if (user.name && user.name.length < 8)
		error.name = 'El nombre debe contener al menos 8 caracteres';
	if (!/^[a-zA-ZÀ-ÿ\u00f1\u00d1\s]+$/.test(user.name))
		error.name1 = 'El nombre debe contener solo letras';
...
function SignUp() {
				...
				if (Object.entries(error).length === 0) {
					try {
						...
						if (userData !== undefined) {
							...
							correct(text);
							await dispatch(getUserDetail(userData.user.email));
						}
						navigate('/feed');
					} catch (error) {
						...
						if (error.code === 'auth/internal-error') {
							//cuando pondo email y no contraseña
	...
	return (
		<>
			<Helmet>
				<title>{t('signup.Night Out - Sign Up')}</title>
			</Helmet>
			<div className="flex flex-col items-center justify-evenly m-20 w-full">
				...
				<div className="bg-gradient-to-r from-gradiante1 via-gradiante2 to-gradiante4 rounded-3xl flex flex-col justify-evenly items-center lg:w-3/4 md:p-10 text-center s:w-11/12 s:px-3 s:py-10 xl:w-1/2 2xl:w-1/3 dark:from-Lgradiante1 dark:via-Lgradiante2 dark:to-Lgradiante4">
					...
					>
						...
						<div className="w-full flex flex-row flex-nowrap bg-gray rounded-full mb-5">
							<img className="h-8 m-3" src={sobre} alt="imagen mail" />
							<input
								className="colorNegro w-full rounded-r-full outline-none text-xl bg-gray"
								type="email"
								name="email"
						...
						<div className="w-full flex flex-row flex-nowrap bg-gray rounded-full mb-5">
							<img className="h-8 m-3" src={dni} alt="imagen dni" />
							<input
						...
						<div className="w-full flex flex-row flex-nowrap bg-gray rounded-full mb-5">
							<img className="h-8 m-3" src={telefono} alt="imagen telefono" />
							<input
								className="colorNegro w-full rounded-r-full outline-none text-xl bg-gray"
								type="tel"
								name="cel"
						...
						<div className="text-white text-xl flex md:flex-row items-center justify-center s:flex-col s:mb-5">
							...
							/>
							<label
								className="m-5 text-center s:order-1 md:order-2"
								htmlFor="tyc"
							>
